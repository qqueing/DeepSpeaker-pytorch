...
@Software: PyCharm
@File: reverberate_data_dir.py
@Time: 2020/3/7 9:31 AM
@Overview:
"""
...
def generate_reverberated_wav_scp(wav_scp,
                                  # a dictionary whose values are the Kaldi-IO strings of the speech recordings
                                  durations,
    ...
    for i in range(start_index, num_replicas + 1):
        for recording_id in keys:
            ...
            reverberate_opts = generate_reverberation_opts(room_dict,
                                                           # the room dictionary, please refer to make_room_dict() for the format
                                                           pointsource_noise_list,  # the point source noise list
                                                           iso_noise_dict,  # the isotropic noise dictionary
                                                           foreground_snrs,  # the SNR for adding the foreground noises
                                                           background_snrs,  # the SNR for adding the background noises
...
def create_reverberated_copy(input_dir,
    ...
    add_prefix_to_fields(input_dir + "/utt2spk", output_dir + "/utt2spk", num_replicas, include_original, prefix,
                         field=[0, 1])
    ...
    if os.path.isfile(input_dir + "/utt2uniq"):
        add_prefix_to_fields(input_dir + "/utt2uniq", output_dir + "/utt2uniq", num_replicas, include_original, prefix,
                             field=[0])
    else:
    ...
    if os.path.isfile(input_dir + "/text"):
        add_prefix_to_fields(input_dir + "/text", output_dir + "/text", num_replicas, include_original, prefix,
                             field=[0])
    if os.path.isfile(input_dir + "/segments"):
        add_prefix_to_fields(input_dir + "/segments", output_dir + "/segments", num_replicas, include_original, prefix,
                             field=[0, 1])
    if os.path.isfile(input_dir + "/reco2file_and_channel"):
        add_prefix_to_fields(input_dir + "/reco2file_and_channel", output_dir + "/reco2file_and_channel", num_replicas,
...
def parse_noise_list(noise_set_para_array, smoothing_weight, sampling_rate=None):
    ...
    for noise_set in set_list:
        ...
        pointsource_noise_list += smooth_probability_distribution(current_pointsource_noise_list, smoothing_weight,
