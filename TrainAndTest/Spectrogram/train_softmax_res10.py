...
from Process_Data.audio_processing import GenerateSpect, concateinputfromMFB, randomLengthFeat
...
parser.add_argument('--resume',
                    default='Data/checkpoint/spectrogram/soft_res10/checkpoint_1.pth',
                    type=str,
                    metavar='PATH',
...
def main():
    ...
    def my_collate(batch):
        ...
        target = []
        for item in batch:
            data = torch.cat((data, item[0].resize(1, item[0].shape[0], item[0].shape[1], item[0].shape[2])), dim=0)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        try:
            minibatch_acc = float((predicted_one_labels.cuda() == true_labels).sum().item()) / len(predicted_one_labels)
        except:
            pdb.set_trace()
        correct += float((predicted_one_labels==true_labels).sum().item())
    ...
    check_path = pathlib.Path('{}/soft_10res/checkpoint_{}.pth'.format(args.ckp_dir, epoch))
