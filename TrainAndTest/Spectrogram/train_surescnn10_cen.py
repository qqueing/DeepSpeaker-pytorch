...
from Define_Model.SoftmaxLoss import AngleSoftmaxLoss, CenterLoss
...
def main():
    ...
    ce2 = CenterLoss(num_classes=len(train_dir.classes), feat_dim=args.embedding_size)
...
def train(train_loader, model, ce, optimizer, scheduler, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        # pdb.set_trace()
        classfier, feat = model(data)
        cos_theta, phi_theta = classfier

        true_labels = label.cuda()

        softmax_loss = ce[0](cos_theta, true_labels)
        center_loss = ce[1](feat, true_labels)
