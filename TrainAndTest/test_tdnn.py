...
from Process_Data.audio_processing import totensor, read_MFB, make_Fbank, conver_to_wav, \
    concateinputfromMFB
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        correct += float((predicted_one_labels.cuda() == label.cuda()).sum().data[0])
        total += label.size(0)

        if batch_idx % args.log_interval == 0:
            pbar.set_description('Test Epoch: {:3d} [{:8d}/{:8d} ({:3.0f}%)]\tLoss: {:.6f} \tGlobal Accuracy: {:.6f}%'.format(
                epoch,
                batch_idx * len(data),
    ...
    print('\33[91mFor test epoch model {}, Accuracy is : {:.8f}%\n\33[0m'.format(epoch, 100. * float(correct)/total))
