...
parser.add_argument('--check-path', default='Data/checkpoint/sires34_soft/norm',
                    help='folder to output model checkpoints')

parser.add_argument('--resume',
                    default='Data/checkpoint/sires34_soft/norm/checkpoint_1.pth',
                    type=str,
                    metavar='PATH',
...
# options for vox2
writer = SummaryWriter('Log/sires34_soft/norm', filename_suffix=str(time.asctime()))
...
def test(test_loader, valid_loader, model, epoch):
    ...
    for batch_idx, (data_a, data_p, label) in pbar:
        ...
        # compute output
        out_a, out_p = model.pre_forward_norm(data_a), model.pre_forward_norm(data_p)
        ...
        if batch_idx % args.log_interval == 0:
            pbar.set_description('Test Epoch: {} [{}/{} ({:.2f}%)]'.format(
    ...
    if args.cos_sim:
        ...
        print('\33[91mFor l2_distance, Test set verification ERR is {:.4f}%, when threshold is {}. Valid set classificaton accuracy is {:.2f}%.\n\33[0m'.format(
                100. * eer, eer_threshold, valid_accuracy))
