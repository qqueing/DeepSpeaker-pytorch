...
from Process_Data.audio_processing import concateinputfromMFB, to4tensor
# Version conflict
...
parser.add_argument('--resume',
                    default='Data/checkpoint/resnet10_asoftmax_avgpool/checkpoint_2.pth',
                    type=str,
                    metavar='PATH',
...
parser.add_argument('--resnet-size', type=int, default=10, metavar='E',
                    help='resnet size to train (default: 10)')
...
writer = SummaryWriter('Log/asoftmax_avg')
...
def main():
    ...
    def my_test_collate(batch):
        # pdb.set_trace()
        data_a = [item[0] for item in batch]
        data_b = [item[1] for item in batch]
        target = [item[2] for item in batch]
        target = torch.LongTensor(target)
        #return torch.stack(batch, 0)
        return [data_a, data_b, target]

    train_loader = torch.utils.data.DataLoader(train_dir, batch_size=args.batch_size, collate_fn=my_collate, shuffle=True, **kwargs)
    test_loader = torch.utils.data.DataLoader(test_dir, batch_size=args.test_batch_size, collate_fn=my_collate, shuffle=False, **kwargs)
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        for sample in data:
            data_a = sample.resize_(sample.shape[1], sample.shape[0], sample.shape[2], sample.shape[3])
            output_a = model(Variable(data_a.float().cuda()))
            output_a = output_a.unsqueeze(0)
            encoding_a = model.encodinglayer(output_a)
            output = torch.cat([output, encoding_a.data], 0)

        output = Variable(output.squeeze())
        loss = model.AngularSoftmaxLoss(output, label)
        ...
        #
        # cross_entropy_loss = model.AngularSoftmaxLoss(data.cuda(), true_labels.cuda())
        ...
        if batch_idx % args.log_interval == 0:
            pbar.set_description('Train Epoch: {:3d} [{:8d}/{:8d} ({:3.0f}%)]\tLoss: {:.6f} \t'.format(
                epoch,
                batch_idx * len(data),
    ...
    torch.save({'epoch': epoch+1,
               ...
               '{}/resnet10_asoftmax_avg/checkpoint_{}.pth'.format(CKP_DIR, epoch))
