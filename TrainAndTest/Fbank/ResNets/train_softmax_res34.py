...
@Time: 19-8-6 下午1:29
@Overview: Train the resnet 34 with ce.
"""
...
parser.add_argument('--check-path', default='Data/checkpoint/resnet34_softmax',
                    help='folder to output model checkpoints')

parser.add_argument('--resume',
                    default='Data/checkpoint/resnet34_softmax/checkpoint_2.pth', type=str, metavar='PATH',
...
writer = SummaryWriter(logdir='Log/softmax_res34', filename_suffix='vox1_fb64')
...
if args.acoustic_feature == 'fbank':
    transform = transforms.Compose([
        ...
        concateinputfromMFB(),  # 300 frames
        totensor()
    ])
...
test_dir = VoxcelebTestset(dir=args.test_dataroot, pairs_path=args.test_pairs_path, loader=file_loader,
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        if batch_idx % args.log_interval == 0:
            pbar.set_description(
                'Train Epoch: {:3d} [{:8d}/{:8d} ({:3.0f}%)]\tLoss: {:.6f} \tBatch Accuracy: {:.6f}%'.format(
                    epoch,
                    batch_idx * len(data),
    ...
    print('\n\33[91mFor epoch {}: Softmax Train set Accuracy:{:.6f}%, and Average loss is {}. \n\33[0m'.format(epoch,
                                                                                                               100 * float(
