...
parser.add_argument('--check-path', default='Data/checkpoint/sires34_soft/adawd',
                    help='folder to output model checkpoints')

parser.add_argument('--resume',
                    default='Data/checkpoint/sires34_soft/adawd/checkpoint_16.pth',
                    type=str,
                    metavar='PATH',
...
# options for vox2
writer = SummaryWriter('Log/sires34_soft/adawd', filename_suffix=str(time.asctime()))
...
def main():
    ...
    # pdb.set_trace()
    train_loader = torch.utils.data.DataLoader(train_dir, batch_size=args.batch_size, collate_fn=PadCollate(dim=2),
                                               shuffle=True, **kwargs)
    valid_loader = torch.utils.data.DataLoader(valid_dir, batch_size=args.test_batch_size, collate_fn=PadCollate(dim=2),
...
def test(test_loader, valid_loader, model, epoch):
    ...
    if args.cos_sim:
        print(
            '\n\33[91mFor cos_distance, Test set ERR is {:.8f}% when threshold is {}. Valid set classificaton accuracy is {:.2f}%.\33[0m'.format(
                100. * eer, eer_threshold, valid_accuracy))
    else:
        print('\33[91mFor l2_distance, Test set ERR: {:.8f}%\tBest ACC:{:.8f}.\33[0m'.format(100. * eer, accuracy))
...
def valid(valid_loader, model, epoch):
    ...
    print('\33[91mValid set classificaton accuracy is {:.2f}%.\33[0m'.format(valid_accuracy))
