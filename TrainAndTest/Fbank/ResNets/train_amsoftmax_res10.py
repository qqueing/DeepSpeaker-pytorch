...
parser.add_argument('--resume',
                    default='Data/checkpoint/resnet10_amsoftmax/checkpoint_2.pth',
                    type=str,
                    metavar='PATH',
...
def train(train_loader, model, optimizer, epoch):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        loss = model.AMSoftmaxLoss(feats, true_labels.cuda())
        ...
        if batch_idx % args.log_interval == 0:
            pbar.set_description(
                'Train Epoch: {:3d} [{:8d}/{:8d} ({:3.0f}%)]\tBatch Loss: {:.6f} \tMinibatch Accuracy: {:.6f}%'.format(
                    epoch,
                    batch_idx * len(data),
